...
def batch_processor(model, data, train_mode, epoch=-1, cal_cov=False, cov_list=None, max_epochs=-1):
    if train_mode:
        if cal_cov:
    ...
    _, losses = model(data, epoch, max_epochs, cov_list)
...
def _dist_train(
    ...
    # build runner
    runner = Runner(
        model, batch_processor, optimizer, cfg.work_dir, logger, cfg=cfg
    )
    ...
    if validate:
        ...
        if eval_dataset is not None:
            ...
            if isinstance(eval_dataset, list):
                logger.info("There are {0:} eval datasets...".format(len(eval_dataset)))
                for single_dataset in eval_dataset:
                    if task == 'stereo':
                        runner.register_hook(DistStereoEvalHook(cfg, single_dataset, eval_interval))
                    elif task == 'flow':
                        runner.register_hook(DistFlowEvalHook(cfg, single_dataset, eval_interval))
            else:
                if task == 'stereo':
                    runner.register_hook(DistStereoEvalHook(cfg, eval_dataset, eval_interval))
                elif task == 'flow':
                    runner.register_hook(DistFlowEvalHook(cfg, eval_dataset, eval_interval))
        if vis_dataset is not None:
            logger.info("Register Visualization hook...")
            vis_interval = cfg.get('vis_interval', 1)
            if isinstance(vis_dataset, list):
                logger.info("There are {0:} visualization datasets...".format(len(vis_dataset)))
                for single_dataset in vis_dataset:
                    if task == 'stereo':
                        runner.register_hook(DistStereoVisHook(single_dataset, cfg, vis_interval))
                    elif task == 'flow':
                        runner.register_hook(DistFlowVisHook(single_dataset, cfg, vis_interval))
            else:
                if task == 'stereo':
                    runner.register_hook(DistStereoVisHook(vis_dataset, cfg, vis_interval))
                elif task == 'flow':
                    runner.register_hook(DistFlowVisHook(vis_dataset, cfg, vis_interval))
