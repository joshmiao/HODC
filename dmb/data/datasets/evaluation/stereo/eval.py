...
def do_occlusion_evaluation(est_disp, ref_gt_disp, target_gt_disp, lb, ub, occMask=None):
    """
    ...
    if torch.is_tensor(ref_gt_disp):
        ref_gt_disp = ref_gt_disp.clone().cuda()
    if torch.is_tensor(target_gt_disp):
        target_gt_disp = target_gt_disp.clone().cuda()
    if torch.is_tensor(occMask):
        occMask = occMask.clone().cuda()
