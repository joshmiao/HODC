...
def disp_evaluation(cfg, disps, data_gpu):
    ...
    data_gpu['occMask'] = data_gpu['occMask'] if 'occMask' in data_gpu else None
    if data_gpu['occMask'] is not None:
        data_gpu['occMask'] = remove_padding(data_gpu['occMask'], ori_size)

    leftDisp = data_gpu['leftDisp']
    rightDisp = data_gpu['rightDisp']
    occMask = data_gpu['occMask']
    ...
    for id in eval_disparity_id:
        ...
        if cfg.model.eval.eval_occlusion and (((leftDisp is not None) and (rightDisp is not None)) or (occMask is not None)):

            noc_occ_error_dict = do_occlusion_evaluation(
                disps[id], leftDisp, rightDisp,
                cfg.model.eval.lower_bound, cfg.model.eval.upper_bound, occMask)
...
class DistStereoEvalHook(DistEvalHook):
    ...
    def evaluate(self, runner, results):
        ...
        for result in results:
            if result is None:
                runner.logger.info('Encounter a result which value is None.')
            elif not 'Error' in result:
                runner.logger.info('Encounter a result which has no attribute named Error.')
            else:
                flag = False
                # print(result)
                for key, var in result['Error'].items():
                    if not isinstance(var, torch.Tensor) or var.size() != torch.Size([]):
                        flag = True
                        break
                if flag:
                    continue
        ...
        for key in error_log_buffer.output.keys():
            runner.log_buffer.output[self.dataset.name + '/' + key] = error_log_buffer.output[key]

        # for better visualization, format into pandas
        format_output_dict = disp_output_evaluation_in_pandas(error_log_buffer.output)

        runner.logger.info("Epoch [{}] Evaluation Result on {} dataset: \t".format(runner.epoch + 1, self.dataset.name))
