...
class FCPSMNetBackbone_woMo(nn.Module):
    """
    Backbone proposed in PSMNet.
    Args:
        in_planes (int): the channels of input
    ...
    def __init__(self, in_planes=3, batch_norm=True, m=0.999):
        super(FCPSMNetBackbone_woMo, self).__init__()
        self.in_planes = in_planes

        self.encoder = PSM_Encoder_Instance(in_planes, batch_norm)
    ...
    def forward(self, *input):
        ...
        l_fms, l_w_arr = self.encoder(l_img)
        r_fms, r_w_arr = self.encoder(r_img)
