...
class ShowDisp(object):
    ...
    def __call__(self, result):
        ...
        if self.estDisp is not None:
            ...
            group = self.vis_group_color(self.estDisp[0], self.gtDisp, self.leftImage, self.rightImage, max_disp=self.max_disp)
        ...
        if self.estDisp is not None and self.gtDisp is not None:
            assert isinstance(self.estDisp[0], (np.ndarray, torch.Tensor))

            if torch.is_tensor(self.estDisp[0]):
                est = self.estDisp[0].clone().detach().cpu().numpy()
            else:
                est = self.estDisp[0]
            ...
            assert isinstance(self.gtDisp, (np.ndarray, torch.Tensor))
            
            if torch.is_tensor(self.gtDisp):
                gt = self.gtDisp.clone().detach().cpu().numpy()
            else:
                gt = self.gtDisp
            ...
            err = disp_err_to_color(est, gt).transpose(2, 0, 1).astype(np.float32) / 255.0
            process_result.update(ErrorMap=err)
    ...
    def vis_group_color(self, estDisp, gtDisp=None, leftImage=None, rightImage=None, save_path=None, max_disp=None):
        """
        Args:
        ...
        return group_color(estDisp, gtDisp, leftImage, rightImage, save_path, max_disp)
