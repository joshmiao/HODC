...
def prepare_visualize(result, epoch, work_dir, image_name, show_origin_data):
    result_tool = ShowResultTool()
    result = result_tool(result, color_map='gray', bins=100)
    mkdir_or_exist(os.path.join(work_dir, 'vis_results', image_name))
    save_path = os.path.join(work_dir, 'vis_results', image_name, '{}.png'.format(epoch))
    ...
    for pred_item in result.keys():
        ...
        if pred_item == 'Disparity':
            if isinstance(result[pred_item], list):
                # only visualize the first output
                log_result['image/' + log_name] = result[pred_item][0]
            else:
                log_result['image/' + log_name] = result[pred_item]
        if pred_item == 'GroundTruth' and show_origin_data:
            log_result['image/' + log_name] = result[pred_item]
        if pred_item == 'ErrorMap':
            log_result['image/' + log_name] = result[pred_item]
    ...
    def after_train_epoch(self, runner):
        ...
        for idx in range(runner.rank, len(self.dataset), runner.world_size):
            ...
            result['leftImage'] = imread(
                osp.join(self.dataset.root, item['left_image_path'])
            ).astype(np.float32)
            result['rightImage'] = imread(
                osp.join(self.dataset.root, item['right_image_path'])
            ).astype(np.float32)

            image_name = self.dataset.name + '/' + item['left_image_path'].split('/')[-2] + item['left_image_path'].split('/')[-1]
            if self.last_show_origin_data == -1 or self.last_show_origin_data == runner.epoch:
                show_origin_data = True
                self.last_show_origin_data = runner.epoch
            else:
                show_origin_data = False
            result = prepare_visualize(result, runner.epoch + 1, self.cfg.work_dir, image_name, show_origin_data)
            
            if show_origin_data:
                result['image/' + image_name + '/leftImage'] = imread(
                    osp.join(self.dataset.root, item['left_image_path'])
                ).astype(np.uint8).transpose((2, 0, 1))
                result['image/' + image_name + '/rightImage'] = imread(
                    osp.join(self.dataset.root, item['right_image_path'])
                ).astype(np.uint8).transpose((2, 0, 1))
