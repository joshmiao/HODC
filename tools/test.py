...
def multi_gpu_test(model, dataset, cfg, show=False, tmpdir=None):
    ...
    for idx in range(rank, len(dataset), world_size):
        ...
        if show:
            ...
            mean, std = torch.tensor([123.675, 116.28, 103.53]), torch.tensor([58.395, 57.12, 57.375])
            result['leftImage'] = data['leftImage'].squeeze(0).permute(1, 2, 0)[-h:, :w, :].cpu()
            result['leftImage'] = (result['leftImage'] * std + mean).detach().numpy()
            
            result['rightImage'] = data['rightImage'].squeeze(0).permute(1, 2, 0)[-h:, :w, :].cpu()
            result['rightImage'] = (result['rightImage'] * std + mean).detach().numpy()
            
            image_name = item['left_image_path'].split('/')[-3] + item['left_image_path'].split('/')[-2] + item['left_image_path'].split('/')[-1]
